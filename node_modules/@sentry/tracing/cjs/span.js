...
class Span  {
...

 {
   ...
   */
   getBaggage() {
    ...
    var finalBaggage =
      !existingBaggage || utils.isSentryBaggageEmpty(existingBaggage)
        ? this._getBaggageWithSentryValues(existingBaggage)
        : existingBaggage;

    return utils.isBaggageEmpty(finalBaggage) ? undefined : finalBaggage;
  }
...

 {
   ...
   * @returns
   */
   _getBaggageWithSentryValues(baggage = utils.createBaggage({})) {
        var hub$1 = ((this.transaction ) && (this.transaction )._hub) || hub.getCurrentHub();
    var client = hub$1.getClient();

    const { environment, release } = (client && client.getOptions()) || {};

    environment && utils.setBaggageValue(baggage, 'environment', environment);
    release && utils.setBaggageValue(baggage, 'release', release);
