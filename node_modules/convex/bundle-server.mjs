...
import { bundle, bundleAll } from "./dist/esm/bundler/index.js";
...
const setupBundles = await bundle(process.argv[2], [setupPath], true);
...
for (const systemDir of systemDirs) {
  ...
  const bundles = await bundleAll(systemDir, false, false);
  out.push(...bundles);
}
