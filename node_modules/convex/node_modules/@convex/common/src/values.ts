...
function bigIntToBase64(value: bigint): string {
  const integerBytes = Long.fromString(value.toString(), false).toBytesLE();
...
function base64ToBigInt(encoded: string): bigint {
  ...
  return BigInt(Long.fromBytesLE(Array.from(integerBytes), false).toString());
}
...
function convexToJsonInternal(
  ...
  if (value === undefined) {
    throw new Error(
      `undefined is not a valid Convex value (present at path ${context} in original object ${JSON.stringify(
        originalValue
  ...
  if (typeof value !== "object") {
    throw new Error(
      `${value} is not a supported Convex type (present at path ${context} in original object ${JSON.stringify(
        originalValue
      )}). To learn about Convex's supported types, see https://docs.convex.dev/using/types.`
    );
  }
