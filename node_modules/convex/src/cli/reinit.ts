...
export const reinit = new Command("reinit")
  .description(
    "Reinitialize a Convex app in the local directory if you've lost your convex.json and .env.local files"
  )
  .addOption(BETA_KEY_OPTION)
  .option(
    "--deployment-name <name>",
    "The name of the deployment you'd like to reinitialize. If you don't know it, you can list all of your deployments with `convex list-deployments`."
  )
  .option(
  ...
  .action(async options => {
    ...
    if (await exists(configFn)) {
      ...
      console.error(
        "If you'd like to regenerate it, delete the file and rerun `convex reinit`"
      );
    ...
    if (deploymentName === undefined) {
      console.error(
        chalk.red(
          `The "--deployment-name" flag is required to reinitialize a project.`
        )
      );
      console.error(
        chalk.red(
          "If you don't know your deployment name, you can list all of your deployments with `convex list-deployments`."
        )
      );
      return await flushAndExit(1);
    }
    ...
    if (deploymentName !== data.instanceName) {
      throw new Error(
        `Reinitialized different deployment ${data.instanceName}, expected ${deploymentName}`
      );
    }
