...
const maybeQuote = (s: string) => (/[":\\/]/.test(s) ? `"${s}"` : s);
const filepathToUdfPath = (filePath: string) => {
  filePath = filePath.replace("\\", "/");
  const lastDot = filePath.lastIndexOf(".");
  return filePath.slice(0, lastDot === -1 ? undefined : lastDot);
};
const filepathToIdentifier = (filePath: string): string =>
  filepathToUdfPath(filePath).replace("/", "_");

const udfPath = ({ filename, name }: Udf) =>
  name === "default"
...
export const udfPathKey = (imp: Udf) => maybeQuote(udfPath(imp));

// foo or foo__bar or foo_bar__baz - must be unique in generated code
const apiName = ({ filename, name }: Udf) =>
  name === "default"
    ? filepathToIdentifier(filename)
    : `${filepathToIdentifier(filename)}__${name}`;

// ./foo or ./foo or ./foo/bar
const importPath = ({ filename }: Udf) => `../${filepathToUdfPath(filename)}`;

export const importStmt = (imp: Udf) =>
  imp.name === "default"
    ? `import type ${apiName(imp)} from "${importPath(imp)}";`
    : `import type { ${imp.name} as ${apiName(imp)} } from "${importPath(
        imp
      )}";`;

export function reactCodegen(udfs: Udf[]) {
  return `${header("Generated React hooks.")}
  ${udfs.map(imp => importStmt(imp)).join("\n")}
  ...
  ${udfs
    ...
    .map(imp => `${udfPathKey(imp)}: ClientQuery<typeof ${apiName(imp)}>;`)
    .join("\n")}
    };
    mutations: {
  ${udfs
    .filter(({ isMutation }) => isMutation)
    .map(imp => `${udfPathKey(imp)}: ClientMutation<typeof ${apiName(imp)}>;`)
    .join("\n")}
    };
  };
