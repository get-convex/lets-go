...
export const init = new Command("init")
  ...
  .option(
    ...
    "Type of deployment to create (`convex.<type>.json`). Defaults to `convex.json`"
  )
  .action(async options => {
    ...
    if (await exists(configPath)) {
      const deploymentTypeFlag =
        options.deploymentType != undefined
          ? ` --deployment-type=${options.deploymentType}`
          : "";
      console.error(chalk.red(`Deployment "${configPath}" already exists.`));

      console.error(
        `Use \`convex push${deploymentTypeFlag}\` to apply changes to the existing deployment.`
      );
      console.error(
        `Use \`convex deactivate${deploymentTypeFlag}\` to deactivate the existing deployment and reclaim a deployment towards your deployment quota.`
      );
      console.error();

      console.error(
        "Use `convex init --deployment-type={type}` to create an additional deployment."
      );
    ...
    const { instanceConfig, modules } = await pullConfig(
      instanceOrigin,
      adminKey
    );
    ...
    console.log(
      chalk.green(
        `Done! Your account now has ${redemptionsRemaining} deployments remaining.`
      )
    );
    {
    ...
    console.log(
      "Please don't use the current version of Convex for any mission-critical applications."
    );
...
async function provision(
  ...
  const headers: Record<string, string> = authHeader
    ...
    : {};
  const provisioningArgs: ProvisionArgs = {
    betaKey,
    existingInstanceName,
    deployment,
...
export async function optins(
  ...
  let res: CheckOptInsResult;
  try {
    const args: CheckOptInsArgs = { betaKey };
  ...
  const args: AcceptOptInsArgs = { betaKey, optInsAccepted };
