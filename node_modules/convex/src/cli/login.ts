...
import { Issuer, errors, BaseClient, custom } from "openid-client";
import {
  globalConfigPath,
  rootDirectory,
  flushAndExit,
...
async function performDeviceAuthorization(
  ...
  console.log(
    `You should see the following code: ${user_code}. It expires in ${
      expires_in % 60 === 0
...
export const login = new Command("login")
  ...
  .action(async (options, cmd: Command) => {
    ...
    const clientId =
      options.overrideAuthClient ?? "HFtA247jp9iNs08NTLIB7JsNPMmRIyfi";
    ...
    try {
      const resp = await client.post(
        `${provisionHost}/api/${version}/authorize`,
        authorizeArgs
      );
