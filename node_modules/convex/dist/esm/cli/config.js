...
import { bundleAll } from "../bundler/index.js";
import { version } from "../index.js";
import { parse as envfileParse } from "envfile";
import axiosRetry from "axios-retry";
import { deprecationCheck, exists, flushAndExit, functionsDir, loadPackageJson, printServerErr, } from "./utils.js";
...
export function configName(deployment) {
    if (deployment == null) {
        return "convex.json";
    }
    else {
...
export async function configFilepath(configFn) {
    ...
    if (standardLocationExists && srcLocationExists) {
        console.error(chalk.red(`Error: both ${preferredLocation} and ${wrongLocation} files exist!`));
    ...
    try {
        ...
        console.error(chalk.red(`Error: use of create-react-app detected (found react-scripts in package.json) but no src directory found.`));
        return await flushAndExit(1);
    }
    return preferredLocation;
}
/** Read configuration from a local `convex.json` file. */
export async function readInstanceConfig(deployment) {
    let instanceConfig;
    const configFn = configName(deployment);
    const configPath = await configFilepath(configFn);
    try {
        instanceConfig = parseConfig(JSON.parse(await fs.readFile(configPath, { encoding: "utf-8" })));
    }
    catch (err) {
        if (err instanceof ParseError || err instanceof SyntaxError) {
            ...
            console.error(chalk.red(`Error: Unable to read deployment config file "${configPath}"`));
 ...
 * complete config.
 */
export async function configFromInstanceConfig(instanceConfig, configPath, verbose) {
    let modules;
    try {
        modules = await bundleAll(functionsDir(configPath, instanceConfig), true, verbose);
    }
    catch (err) {
 ...
 */
export async function readConfig(deployment, verbose) {
    const { instanceConfig, configPath } = await readInstanceConfig(deployment);
    const config = await configFromInstanceConfig(instanceConfig, configPath, verbose);
    return { config, configPath };
}
export async function mustReadEnv(deployment) {
...
export async function readEnv(deployment) {
...
export async function writeReadme(functionsDir) {
    ...
    return client.get(readmeUrl).then(async (res) => {
        ...
        try {
            ...
            console.error(chalk.red('Error: Unable to open file "README.md" for writing in "' +
                functionsDir +
                '"'));
            console.error("Are you running this command from the root directory of a Convex app?");
            return await flushAndExit(1);
...
export async function writeInstanceConfig(deployment, instanceConfig) {
    const configFn = configName(deployment);
    const configPath = await configFilepath(configFn);
    let configFile;
    try {
        ...
        console.error(chalk.red(`Error: Unable to open deployment config file "${configPath}" for writing in current directory`));
        console.error("Are you running this command from the root directory of a Convex app?");
        return await flushAndExit(1);
    }
    try {
        await configFile.writeFile(JSON.stringify(instanceConfig, undefined, 2) + "\n");
    }
    catch (err) {
        console.error(chalk.red(`Error: Unable to write deployment config file "${configPath}"`));
        return await flushAndExit(1, err);
    }
    finally {
        configFile.close();
...
export async function writeAdminKey(deployment, adminKey) {
    if (!adminKey) {
    ...
    try {
        ...
        console.error(chalk.red(`Error: Unable to open file "${envFn}" in current directory`));
        console.error("Are you running this command from the root directory of a Convex app?");
        return await flushAndExit(1);
    }
    try {
        ...
        console.error(chalk.red(`Error: Unable to write file "${envFilename}" in current directory`));
        return await flushAndExit(1, err);
    }
    finally {
        envLocalFile.close();
...
export async function pullConfig(origin, adminKey) {
    const client = axios.create();
    axiosRetry(client, {
        retries: 4,
    ...
    try {
        const res = await client.post(`${origin}/api/${version}/get_config`, { version, adminKey }, {
        ...
        console.error(chalk.red("Error: Unable to pull deployment config from", origin));
        printServerErr(err);
        return await flushAndExit(1, err);
    }
}
...
export async function pushConfig(config, adminKey) {
    ...
    try {
        ...
        console.error(chalk.red("Error: Unable to push deployment config to", origin));
        printServerErr(err);
        return await flushAndExit(1, err);
    }
}
...
export function diffConfig(oldConfig, newConfig, configPath, verbose) {
    let diff = "";
    const droppedModules = [];
    for (const oldModule of oldConfig.modules) {
        let matches = false;
        for (const newModule of newConfig.modules) {
            const oldPath = path.join(oldConfig.instanceConfig.functions, oldModule.path);
            const newPath = path.join(newConfig.instanceConfig.functions, newModule.path);
            if (oldPath === newPath &&
    ...
    if (droppedModules.length > 0) {
        ...
        for (const module of droppedModules) {
            diff +=
                "[-] " +
                    renderModule(functionsDir(configPath, oldConfig.instanceConfig), module, verbose) +
                    "\n";
        }
    }
    const addedModules = [];
    ...
    if (addedModules.length > 0) {
        ...
        for (const module of addedModules) {
            diff +=
                "[+] " +
                    renderModule(functionsDir(configPath, newConfig.instanceConfig), module, verbose) +
                    "\n";
        }
    }
    const droppedAuth = [];
