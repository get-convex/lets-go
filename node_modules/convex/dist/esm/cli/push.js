...
import { mustReadEnv, pullConfig, pushConfig, diffConfig, readInstanceConfig, configFromInstanceConfig, configJSON, } from "./config.js";
import { flushAndExit, functionsDir, pkgVersionConsistencyCheck, } from "./utils.js";
...
export const push = new Command("push")
    ...
    .addOption(new Option("--typecheck <mode>", `Whether to check TypeScript files with \`tsc --noEmit\` before pushing.`)
    .choices(["enable", "try", "disable"])
    .default("disable"))
    .addOption(new Option("--debug").hideHelp())
    .addOption(new Option("--codegen <mode>", "Whether to regenerate client code on push.")
    ...
    const { configPath, instanceConfig } = await readInstanceConfig(options.deploymentType);
    ...
    if (options.codegen === "disable") {
        console.error(chalk.gray("Skipping codegen. Remove --codegen=disable to enable."));
        ...
        await processTypeCheckResult(options.typecheck, () => typeCheckFunctions(funcDir));
        localConfig = await configFromInstanceConfig(instanceConfig, configPath, options.verbose);
    }
    else {
    ...
    await buildIndexes(localConfig.instanceConfig.origin, adminKey, functionsDir(configPath, localConfig.instanceConfig), !!options.dryRun);
    const remoteConfig = await pullConfig(origin, adminKey);
    const diff = diffConfig(remoteConfig, localConfig, configPath, options.verbose);
    if (diff == "") {
        console.log(chalk.gray(`${options.dryRun
            ? "Command would skip function push"
            : "Function push skipped"}: Deployment config already synced.`));
        return await flushAndExit(0);
    }
    console.log(chalk.bold(`Remote deployment config ${options.dryRun ? "would" : "will"} be overwritten with the following changes:`));
    console.log(diff);
    if (options.dryRun) {
        return;
    }
