import { Command } from "commander";
import { loadPackageJson } from "./utils.js";
import chalk from "chalk";
export const update = new Command("update")
    .description("Print instructions to update the convex package")
    .action(async () => {
    let updateInstructions = "npm install convex@latest\n";
    const oldPackages = (await loadPackageJson()).filter((elt) => elt.name.startsWith("@convex-dev"));
    for (const pkg of oldPackages) {
        updateInstructions += `npm uninstall ${pkg.name}\n`;
    }
    console.log(chalk.green(`To view the Convex changelog, go to https://blog.convex.dev/tag/releases/\nWhen you are ready to upgrade, run the following commands:\n${updateInstructions}`));
});
//# sourceMappingURL=update.js.map