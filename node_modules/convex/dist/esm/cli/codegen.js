...
import { configFromInstanceConfig, configJSON, mustReadEnv, readInstanceConfig, } from "./config.js";
import chalk from "chalk";
import { deprecationCheck, flushAndExit, functionsDir, pkgVersionConsistencyCheck, printServerErr, sorted, } from "./utils.js";
import { version } from "../index.js";
import { jsonToConvex } from "@convex-dev/common";
import { reactCodegen } from "./codegen_templates/react.js";
import { dataModelCodegen, dataModelCodegenWithoutSchema, } from "./codegen_templates/dataModel.js";
import { serverCodegen } from "./codegen_templates/server.js";
import { doTypeCheck, processTypeCheckResult, typeCheckFunctions, } from "./typecheck.js";
export const codegen = new Command("codegen")
    ...
    .addOption(new Option("--typecheck <mode>", `Whether to check TypeScript files with \`tsc --noEmit\`.`)
    ...
    if (options.typecheck !== "disable") {
        console.error(chalk.gray("Trying experimental TypeScript typecheck, remove --typecheck=enable to disable."));
    }
    await doCodegen({
        instanceConfig,
        configPath,
 ...
 */
async function determineFunctions(config, adminKey) {
    const serializedConfig = configJSON(config, adminKey);
    try {
        const res = await axios.post(`${config.instanceConfig.origin}/api/${version}/analyze_config`, serializedConfig);
        deprecationCheck(res);
        return jsonToConvex(res.data);
    }
    catch (err) {
        console.error(chalk.red("Error: Unable to determine Convex functions", config.instanceConfig.origin));
        printServerErr(err);
...
function doServerCodegen(codegenDir, dryRun, hasSchemaFile, debug) {
    if (hasSchemaFile) {
        ...
        writeFile("dataModel.ts", dataModelCodegenWithoutSchema(), codegenDir, dryRun, debug);
    }
    writeFile("server.ts", serverCodegen(), codegenDir, dryRun, debug);
}
async function doReactCodegen(config, adminKey, codegenDir, dryRun, debug) {
    const { modules } = await determineFunctions(config, adminKey);
    const udfs = [];
    const filenames = [...modules.keys()].sort();
    for (const filename of filenames) {
        const udfsInModule = sorted(modules.get(filename).functions, udf => udf.name);
...
export async function doCodegen({ instanceConfig, configPath, adminKey, typeCheckMode, dryRun = false, verbose = false, warnOnMissingSchemaFile = true, debug = false, }) {
    ...
    if (warnOnMissingSchemaFile && !hasSchemaFile) {
        console.warn(chalk.yellow(`Did not find a schema file at \`${schemaPath}\`. The generated code will consider documents to have the \`any\` type. To add a schema see https://docs.convex.dev/using/schemas`));
    }
    ...
    if (hasSchemaFile) {
        await processTypeCheckResult(typeCheckMode, () => doTypeCheck([path.join(funcDir, "schema.ts")]));
    }
    ...
    await processTypeCheckResult(typeCheckMode, () => typeCheckFunctions(funcDir));
