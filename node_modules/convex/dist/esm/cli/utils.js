 ...
 */
export async function flushAndExit(exitCode, err) {
    if (err) {
...
export function printServerErr(err) {
    const res = err.response;
    if (res) {
        console.error(chalk.gray(`${res.status} ${res.statusText}: ${res.data.code}: ${res.data.message}`));
...
export function deprecationCheck(resp) {
...
export async function loadPackageJson() {
    ...
    try {
        ...
        console.error(chalk.red(`Unable to read your package.json: ${err}. Make sure you're running this command from the root directory of a Convex app that contains the package.json`));
...
export async function pkgVersionConsistencyCheck(origin) {
    const convexPackages = (await loadPackageJson()).filter(isConvexPackage);
...
export const isConvexPackage = (elt) => elt.name.startsWith("@convex-dev") || elt.name.startsWith("convex");
...
export function functionsDir(configPath, instanceConfig) {
...
async function readGlobalConfig() {
    ...
    try {
        ...
        console.error(chalk.red(`Failed to read global config in ${configPath}. Please run \`npx convex login\` to generate your global config.`));
        return null;
    }
    try {
        ...
        console.error(chalk.red(`Failed to parse global config in ${configPath} with error ${err}. Please run \`npx convex login\` to regenerate your global config.`));
        return null;
    }
}
...
export const BETA_KEY_DEPRECATION_MESSAGE = "Convex beta keys are deprecated. Please run `npx convex login` to log in.";
export const BETA_KEY_OPTION = new Option("--beta-key [key]", `Convex beta key for the closed beta. ${BETA_KEY_DEPRECATION_MESSAGE}`).hideHelp();
/**
 ...
 */
export const poll = async function (fetch, condition, waitMs = 1000) {
